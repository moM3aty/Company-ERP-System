...
class DeliveryNoteController extends Controller
{
    ...
    public function index(Request $request, Response $response): Response
    {
        ...
        try {
            ...
            $params = [];
            if ($search !== '') {
                $whereClause = "WHERE dn.delivery_number LIKE ? OR dn.customer_name LIKE ? OR w.name_ar LIKE ?";
                $like = "%{$search}%";
    ...
    public function store(Request $request, Response $response): Response
    {
        ...
        try {
            if (empty($data['warehouse_id']) || empty($data['customer_name'])) {
                throw new Exception("يجب تحديد المستودع واسم العميل.");
            }
            ...
            $stmt = $this->db->prepare("INSERT INTO delivery_notes (company_id, delivery_number, warehouse_id, customer_name, delivery_date, status, notes) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$companyId, $delNum, $data['warehouse_id'], trim($data['customer_name']), $data['delivery_date'], $status, $data['notes'] ?? null]);
            ...
            if (!empty($data['product_id']) && is_array($data['product_id'])) {
                foreach ($data['product_id'] as $idx => $prodId) {
                    ...
                    if ($status === 'delivered') {
                        $stmtMov->execute([$companyId, $prodId, $data['warehouse_id'], 'out', 'sale', $delNum, $qty]);
                    }
                }
            }

            $this->db->commit();
            $_SESSION['flash_msg'] = "تم إنشاء إذن التسليم بنجاح.";
        } catch (Throwable $e) {
            if ($this->db->inTransaction()) $this->db->rollBack();
            $_SESSION['flash_err'] = "خطأ: " . $e->getMessage();
            return new RedirectResponse('/ERP/inventory/delivery-notes/create');
        }

        return new RedirectResponse('/ERP/inventory/delivery-notes');
    }
    ...
    public function edit(Request $request, Response $response, $id = null): Response
    {
        ...
        try {
            ...
            $stmt = $this->db->prepare("SELECT * FROM delivery_notes WHERE id = ?");
            $stmt->execute([$id]);
    ...
    public function update(Request $request, Response $response, $id = null): Response
    {
        ...
        try {
            ...
            $stmt = $this->db->prepare("UPDATE delivery_notes SET warehouse_id=?, customer_name=?, delivery_date=?, status=?, notes=? WHERE id=? AND status != 'delivered'");
            $stmt->execute([$data['warehouse_id'], trim($data['customer_name']), $data['delivery_date'], $status, $data['notes'] ?? null, $id]);
            ...
            $this->db->commit();
            $_SESSION['flash_msg'] = "تم تحديث إذن التسليم بنجاح.";
        } catch (Throwable $e) {
            if ($this->db->inTransaction()) $this->db->rollBack();
            $_SESSION['flash_err'] = "خطأ: " . $e->getMessage();
            return new RedirectResponse("/ERP/inventory/delivery-notes/{$id}/edit");
        }
    ...
    public function delete(Request $request, Response $response, $id = null): Response
    {
        ...
        try {
            ...
            $stmt = $this->db->prepare("SELECT status FROM delivery_notes WHERE id = ?");
            $stmt->execute([$id]);
            ...
            $this->db->prepare("DELETE FROM delivery_notes WHERE id = ?")->execute([$id]);
