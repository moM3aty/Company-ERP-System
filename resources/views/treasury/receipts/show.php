...
$voucherNumber = htmlspecialchars((string)($receipt->voucher_number ?? '---'));
$receiptDate   = htmlspecialchars((string)($receipt->receipt_date ?? date('Y-m-d')));
$payerName     = htmlspecialchars((string)($receipt->payer_name ?: ($receipt->customer_name ?? ($isAr ? 'عام' : 'General'))));
$accountName   = htmlspecialchars((string)($receipt->account_name ?? ($isAr ? 'الخزينة العامة' : 'Main Safe')));
$accountCode   = htmlspecialchars((string)($receipt->account_code ?? '---'));
$referenceNo   = htmlspecialchars((string)($receipt->reference_no ?? ''));
$description   = htmlspecialchars((string)($receipt->description ?? ''));
$amount        = (float)($receipt->amount ?? 0);
$methodKey     = (string)($receipt->payment_method ?? 'cash');

$methodsMap = [
    'cash'          => $isAr ? 'نقداً (Cash)' : 'Cash',
    'bank_transfer' => $isAr ? 'تحويل بنكي' : 'Bank Transfer',
    'cheque'        => $isAr ? 'شيك مصرفي' : 'Bank Cheque',
...
$t = [
    'ar' => [
        ...
        'amt_label' => 'المبلغ المقبوض / Amount',
        'amt_sub' => 'فقط وقدره مفصلاً بالكامل',
        'payer_label' => 'استلمنا من الفاضل:',
        'acc_label' => 'حساب الصندوق/البنك:',
        'method_label' => 'وسيلة التحصيل:',
        'desc_label' => 'وذلك عن / البيان:',
        'ref_label' => 'مرجع/شيك:',
        'sig_payer' => 'المسلم / المستلم منه',
    ...
    'en' => [
        ...
        'amt_label' => 'Received Amount',
        'amt_sub' => 'Only the amount detailed above',
        'payer_label' => 'Received From:',
        'acc_label' => 'Account / Safe:',
...
<style>
    ...
    .receipt-show-wrapper { max-width: 850px; margin: 0 auto; padding-bottom: 50px; font-family: <?= $isAr ? "'Cairo', sans-serif" : "'Inter', sans-serif" ?>; }
    ...
    .btn-action:hover { background: #f0fdf4; color: var(--c-rec-dark); border-color: #99f6e4; }
    .btn-print { background: var(--c-rec-dark); color: #ffffff; border: none; padding: 10px 24px; border-radius: 10px; font-weight: 800; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; transition:0.2s; }
    ...
    .voucher-title-badge { background: var(--c-rec); color: #fff; padding: 6px 20px; border-radius: 8px; font-size: 1.1rem; font-weight: 900; letter-spacing: 1px; }
    ...
    @media print {
        ...
        .receipt-show-wrapper, .receipt-show-wrapper * { visibility: visible !important; }
        .receipt-show-wrapper { position: absolute !important; left: 0 !important; top: 0 !important; width: 100% !important; max-width: 100% !important; margin: 0 !important; }
...
<div class="receipt-show-wrapper" dir="<?= $dir ?>">
    <div class="header-bar">
        <div style="display:flex; align-items:center; gap:16px;">
            <a href="/ERP/treasury/receipts" class="btn-action"><i class="ph-bold <?= $isAr ? 'ph-arrow-right' : 'ph-arrow-left' ?>"></i></a>
            <div>
                <h2 style="margin:0; font-size:1.6rem; color:var(--c-text-dark); font-weight:900;"><?= $voucherNumber ?></h2>
                <p style="margin:4px 0 0 0; color:var(--c-text-muted); font-size:0.9rem;"><?= $receiptDate ?></p>
            </div>
        </div>
        <div style="display:flex; gap:10px;">
            <a href="/ERP/treasury/receipts/<?= (int)$receipt->id ?>/edit" class="btn-action" style="background:#f1f5f9; color:#0f172a; border:1px solid #cbd5e1;"><i class="ph-bold ph-pencil-simple"></i></a>
    ...
    <div class="print-canvas voucher-card">
        <div class="voucher-header">
            ...
            <div style="text-align:<?= $isAr ? 'left' : 'right' ?>;">
                ...
                <div style="margin-top:8px; font-family:monospace; font-weight:900; color:var(--c-rec-dark); font-size:1.1rem;"># <?= $voucherNumber ?></div>
            </div>
        </div>

        <div class="amount-box">
            <div>
                <span style="font-size:0.8rem; font-weight:800; color:#047857; text-transform:uppercase;"><?= $t['amt_label'] ?></span>
                <div style="font-size:0.9rem; font-weight:700; color:#065f46; margin-top:2px;"><?= $t['amt_sub'] ?></div>
            </div>
        ...
        <div style="margin-top:28px;">
            <div class="info-row">
                <span class="info-label"><?= $t['payer_label'] ?></span>
                <span class="info-val"><?= $payerName ?></span>
            </div>
            <div class="info-row">
