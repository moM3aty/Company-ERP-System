...
$t = [
    'ar' => [
        'title_new' => 'إنشاء حساب مالي جديد', 'title_edit' => 'تعديل بيانات الحساب', 'desc' => 'إعداد وتكوين بطاقة الحساب وربطها بالدليل المحاسبي.',
        'sec_basic' => 'البيانات الأساسية للحساب', 'code' => 'رمز الحساب (الكود)', 'type' => 'طبيعة الحساب (النوع)',
        'type_ast' => 'أصول (Assets) - مدين', 'type_lib' => 'التزامات (Liabilities) - دائن', 'type_eqt' => 'حقوق ملكية (Equity) - دائن', 'type_rev' => 'إيرادات (Revenues) - دائن', 'type_exp' => 'مصروفات (Expenses) - مدين',
        'name_ar' => 'اسم الحساب (عربي)', 'name_en' => 'اسم الحساب (إنجليزي)',
        'sec_struct' => 'الهيكلة المالية والرصيد', 'parent' => 'الحساب الرئيسي (الأب)', 'parent_null' => '-- حساب رئيسي مستقل (جذر) --',
        'branch' => 'الفرع المخصص', 'general_branch' => '-- حساب عام (لكل الفروع) --',
        'op_bal' => 'الرصيد الافتتاحي', 'sec_settings' => 'خصائص وإعدادات الحساب',
        'is_parent' => 'حساب تجميعي (رئيسي)', 'is_parent_desc' => 'يمنع تسجيل قيود يومية عليه، مخصص لتجميع أرصدة الفرعيات.',
        'is_active' => 'حالة الحساب (نشط)', 'is_active_desc' => 'السماح باستخدامه في الفواتير وإصدار القيود المالية.',
        'cancel' => 'إلغاء وتراجع', 'save' => 'اعتماد وإنشاء الحساب', 'update' => 'تحديث وحفظ التعديلات'
    ],
    'en' => [
        'title_new' => 'Create New Account', 'title_edit' => 'Edit Account Data', 'desc' => 'Configure account card and link it to the Chart of Accounts.',
        'sec_basic' => 'Basic Account Information', 'code' => 'Account Code', 'type' => 'Account Nature (Type)',
        'type_ast' => 'Assets - Debit', 'type_lib' => 'Liabilities - Credit', 'type_eqt' => 'Equity - Credit', 'type_rev' => 'Revenues - Credit', 'type_exp' => 'Expenses - Debit',
        'name_ar' => 'Account Name (AR)', 'name_en' => 'Account Name (EN)',
        'sec_struct' => 'Structure & Balance', 'parent' => 'Parent Account', 'parent_null' => '-- Independent Root Account --',
        'branch' => 'Assigned Branch', 'general_branch' => '-- General Account (All Branches) --',
        'op_bal' => 'Opening Balance', 'sec_settings' => 'Account Settings & Properties',
        'is_parent' => 'Parent (Control) Account', 'is_parent_desc' => 'Prevents direct journal entries, used only to sum sub-accounts.',
        'is_active' => 'Account Status (Active)', 'is_active_desc' => 'Allow usage in invoices and financial journals.',
        'cancel' => 'Cancel', 'save' => 'Create Account', 'update' => 'Update Account'
    ]
][$isRtl ? 'ar' : 'en'];
?>
...
<style>
    ...
    .back-btn:hover { background: var(--brand-primary-light); color: var(--brand-primary); border-color: #6ee7b7; transform: translateX(<?= $isRtl ? '4px' : '-4px' ?>); }
    ...
    .form-control:focus { border-color: var(--brand-primary); box-shadow: 0 0 0 4px rgba(5, 150, 105, 0.15); outline: none; background: var(--surface);}
    ...
    .btn-submit { background: linear-gradient(135deg, var(--brand-primary), var(--brand-primary-dark)); color: white; border: none; padding: 14px 32px; border-radius: 12px; font-weight: 900; font-size: 1rem; cursor: pointer; display: inline-flex; align-items: center; gap: 10px; box-shadow: 0 8px 20px rgba(5, 150, 105, 0.25); transition: 0.3s; }
    .btn-submit:hover { transform: translateY(-3px); box-shadow: 0 12px 25px rgba(5, 150, 105, 0.35); }
...
<div class="form-wrapper" dir="<?= $isRtl ? 'rtl' : 'ltr' ?>">
    <div class="glass-header">
        <div style="display:flex; align-items:center; gap:18px;">
            <a href="/ERP/accounting/chart-of-accounts" class="back-btn"><i class="ph-bold <?= $isRtl ? 'ph-arrow-right' : 'ph-arrow-left' ?>"></i></a>
            <div>
    ...
    <form action="<?= $actionUrl ?>" method="POST">
        ...
        <div class="form-section">
            <h3 class="section-title"><i class="ph-fill ph-info" style="color:var(--brand-primary);"></i> <?= $t['sec_basic'] ?></h3>
            <div class="grid-2">
                <div class="form-group">
                    <label class="input-label"><?= $t['code'] ?> <span style="color:var(--brand-danger);">*</span></label>
                    <input type="text" name="code" class="form-control" style="font-family:monospace; font-size:1.2rem; color:var(--brand-primary-dark);" value="<?= $isEdit ? htmlspecialchars($account->code ?? '') : '' ?>" required>
                </div>
                <div class="form-group">
                    <label class="input-label"><?= $t['type'] ?> <span style="color:var(--brand-danger);">*</span></label>
                    <select name="type" class="form-control" required>
                        <?php $actType = $isEdit ? ($account->type ?? '') : ''; ?>
                        <option value="" disabled <?= !$isEdit ? 'selected' : '' ?>>-- Select --</option>
                        <option value="asset" <?= $actType==='asset'?'selected':'' ?>><?= $t['type_ast'] ?></option>
                        <option value="liability" <?= $actType==='liability'?'selected':'' ?>><?= $t['type_lib'] ?></option>
                        <option value="equity" <?= $actType==='equity'?'selected':'' ?>><?= $t['type_eqt'] ?></option>
                        <option value="revenue" <?= $actType==='revenue'?'selected':'' ?>><?= $t['type_rev'] ?></option>
                        <option value="expense" <?= $actType==='expense'?'selected':'' ?>><?= $t['type_exp'] ?></option>
                    </select>
                </div>
                <div class="form-group">
                    <label class="input-label"><?= $t['name_ar'] ?> <span style="color:var(--brand-danger);">*</span></label>
                    <input type="text" name="name_ar" class="form-control" value="<?= $isEdit ? htmlspecialchars($account->name_ar ?? '') : '' ?>" required>
                </div>
                <div class="form-group">
                    <label class="input-label"><?= $t['name_en'] ?></label>
                    <input type="text" name="name_en" class="form-control" value="<?= $isEdit ? htmlspecialchars($account->name_en ?? '') : '' ?>">
                </div>
            </div>
        </div>

        <div class="form-section">
            <h3 class="section-title"><i class="ph-fill ph-tree-structure" style="color:var(--brand-primary);"></i> <?= $t['sec_struct'] ?></h3>
            <div class="grid-3">
                <div class="form-group">
                    ...
                    <select name="branch_id" class="form-control">
                        ...
                        ?>
                            <option value="<?= $b->id ?>" <?= ($isEdit && ($account->branch_id ?? 0) == $b->id) ? 'selected' : '' ?>><?= htmlspecialchars($bName) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="parent_id" class="form-control">
                        ...
                        <?php foreach($parentAccounts ?? [] as $pa): 
                            $paNameEn = property_exists($pa, 'name_en') && !empty($pa->name_en) ? $pa->name_en : ($pa->name_ar ?? '');
                            $paName = $isRtl ? ($pa->name_ar ?? '') : $paNameEn;
                        ?>
                            <option value="<?= $pa->id ?>" <?= ($isEdit && ($account->parent_id ?? '') == $pa->id) ? 'selected' : '' ?>>
                                <?= htmlspecialchars($pa->code ?? '') ?> - <?= htmlspecialchars($paName) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    <label class="input-label"><?= $t['op_bal'] ?> (<?= $currency ?>)</label>
                    <input type="number" step="0.01" name="opening_balance" class="form-control" style="font-family:monospace; font-weight:900; font-size:1.15rem;" value="<?= $isEdit ? ($account->opening_balance ?? '0.00') : '0.00' ?>" <?= $isEdit ? 'readonly title="لا يمكن تعديل الرصيد الافتتاحي"' : '' ?>>
                </div>
            </div>
        </div>

        <div class="form-section">
            <h3 class="section-title"><i class="ph-fill ph-sliders-horizontal" style="color:var(--brand-primary);"></i> <?= $t['sec_settings'] ?></h3>
            <div class="grid-2">
        ...
        <div class="sticky-footer">
            <a href="/ERP/accounting/chart-of-accounts" class="btn-cancel"><?= $t['cancel'] ?></a>
