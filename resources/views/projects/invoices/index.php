...
$t = [
    'ar' => [
        'title' => 'المستخلصات والفواتير', 'desc' => 'إصدار ومتابعة مستخلصات الإنجاز الجارية والختامية والدفعات المقدمة للمشاريع.',
        'add_btn' => 'إنشاء مستخلص / فاتورة', 'col_num' => 'رقم المستخلص', 'col_proj' => 'المشروع والعميل',
        'col_type' => 'النوع', 'col_date' => 'تاريخ الاصدار', 'col_net' => 'الصافي المستحق', 'col_paid' => 'المسدد / المتبقي',
        'col_status' => 'الحالة', 'col_actions' => 'إجراءات', 'empty' => 'لا توجد مستخلصات تطابق بحثك.',
        'search' => 'ابحث برقم المستخلص، المشروع، الوصف...', 'btn_search' => 'بحث', 'clear' => 'إلغاء',
        'stat_total' => 'إجمالي المستخلصات', 'stat_net' => 'صافي القيمة الكلية', 'stat_paid' => 'المحصل والمسدد', 'stat_rem' => 'المتبقي غير المحصل',
        'st_draft' => 'مسودة', 'st_submitted' => 'مقدم للاعتماد', 'st_approved' => 'معتمد للصرف', 'st_partially_paid' => 'مدفوع جزئياً', 'st_paid' => 'مسدد بالكامل', 'st_rejected' => 'مرفوض',
        'tp_adv' => 'دفعة مقدمة', 'tp_prog' => 'مستخلص جاري', 'tp_final' => 'مستخلص ختامي', 'tp_ret' => 'إفراج عن محتجزات',
        'paid' => 'مسدد:', 'rem' => 'متبقي:', 'confirm_del' => 'تأكيد الحذف؟'
    ],
    'en' => [
        'title' => 'Progress Claims & Invoices', 'desc' => 'Issue and track project progress claims, final invoices, and advance payments.',
        'add_btn' => 'Create Claim / Invoice', 'col_num' => 'Claim No.', 'col_proj' => 'Project & Client',
        'col_type' => 'Type', 'col_date' => 'Issue Date', 'col_net' => 'Net Amount', 'col_paid' => 'Paid / Remaining',
        'col_status' => 'Status', 'col_actions' => 'Actions', 'empty' => 'No claims found.',
        'search' => 'Search by claim no, project, desc...', 'btn_search' => 'Search', 'clear' => 'Clear',
        'stat_total' => 'Total Claims', 'stat_net' => 'Total Net Value', 'stat_paid' => 'Collected / Paid', 'stat_rem' => 'Remaining Unpaid',
        'st_draft' => 'Draft', 'st_submitted' => 'Submitted', 'st_approved' => 'Approved', 'st_partially_paid' => 'Partially Paid', 'st_paid' => 'Fully Paid', 'st_rejected' => 'Rejected',
        'tp_adv' => 'Advance Payment', 'tp_prog' => 'Progress Claim', 'tp_final' => 'Final Claim', 'tp_ret' => 'Retention Release',
        'paid' => 'Paid:', 'rem' => 'Rem:', 'confirm_del' => 'Confirm delete?'
    ]
][$isRtl ? 'ar' : 'en'];

$statusMap = [
    'draft' => ['label' => $t['st_draft'], 'color' => '#64748b', 'bg' => '#f1f5f9', 'icon' => 'ph-pencil-line'],
    'submitted' => ['label' => $t['st_submitted'], 'color' => '#d97706', 'bg' => '#fef3c7', 'icon' => 'ph-paper-plane-tilt'],
    'approved' => ['label' => $t['st_approved'], 'color' => '#0284c7', 'bg' => '#e0f2fe', 'icon' => 'ph-check-circle'],
    'partially_paid' => ['label' => $t['st_partially_paid'], 'color' => '#8b5cf6', 'bg' => '#f5f3ff', 'icon' => 'ph-chart-pie-slice'],
    'paid' => ['label' => $t['st_paid'], 'color' => '#059669', 'bg' => '#ecfdf5', 'icon' => 'ph-money'],
    'rejected' => ['label' => $t['st_rejected'], 'color' => '#dc2626', 'bg' => '#fef2f2', 'icon' => 'ph-x-circle'],
];

$typeMap = [
    'advance_payment' => ['label' => $t['tp_adv'], 'bg' => '#eff6ff', 'color' => '#2563eb'],
    'progress_claim' => ['label' => $t['tp_prog'], 'bg' => '#ecfdf5', 'color' => '#059669'],
    'final_claim' => ['label' => $t['tp_final'], 'bg' => '#fdf4ff', 'color' => '#c026d3'],
    'retention_release' => ['label' => $t['tp_ret'], 'bg' => '#fef3c7', 'color' => '#d97706'],
];
?>

<style>
    :root { --c-pi: #059669; --c-pi-dark: #047857; --c-pi-light: #ecfdf5; --c-border: #cbd5e1; --c-text-dark: #0f172a; --c-text-muted: #64748b; }
    ...
    .btn-pi { background: linear-gradient(135deg, var(--c-pi), var(--c-pi-dark)); color: #ffffff !important; border: none; padding: 10px 24px; border-radius: 10px; font-weight: 800; display: inline-flex; align-items: center; gap: 8px; text-decoration: none; box-shadow: 0 4px 12px rgba(5, 150, 105, 0.25); transition:0.2s;}
    .btn-pi:hover { transform: translateY(-2px); box-shadow: 0 6px 16px rgba(5, 150, 105, 0.35); }
    ...
    .form-control:focus { outline:none; border-color: var(--c-pi); }
    ...
    .pi-table tr:hover td { background: #f8fafc; }
...
<div class="pi-wrapper" dir="<?= $isRtl ? 'rtl' : 'ltr' ?>">
    <div class="pi-header">
        <div class="pi-title-box">
            ...
            <div>
                <h2 class="pi-title"><?= $t['title'] ?></h2>
                <p style="margin:4px 0 0 0; color:var(--c-text-muted);"><?= $t['desc'] ?></p>
            </div>
        </div>
        <a href="/ERP/projects/invoices/create" class="btn-pi"><i class="ph-bold ph-plus"></i> <?= $t['add_btn'] ?></a>
    </div>
    ...
    <div class="kpi-row">
        <div class="kpi-card"><div class="kpi-icon"><i class="ph-duotone ph-files"></i></div><div class="kpi-info"><h4><?= $t['stat_total'] ?></h4><p><?= number_format($stats->total_invoices ?? 0) ?></p></div></div>
        <div class="kpi-card" style="border-bottom: 3px solid var(--c-pi);"><div class="kpi-icon" style="background:var(--c-pi-light); color:var(--c-pi);"><i class="ph-duotone ph-currency-circle-dollar"></i></div><div class="kpi-info"><h4 style="color:var(--c-pi);"><?= $t['stat_net'] ?></h4><p><?= number_format((float)($stats->total_net_amount ?? 0), 2) ?> <span style="font-size:0.7rem; color:#64748b;"><?= $currency ?></span></p></div></div>
        <div class="kpi-card" style="border-bottom: 3px solid #2563eb;"><div class="kpi-icon" style="background:#eff6ff; color:#2563eb;"><i class="ph-duotone ph-check-square"></i></div><div class="kpi-info"><h4 style="color:#2563eb;"><?= $t['stat_paid'] ?></h4><p><?= number_format((float)($stats->total_paid ?? 0), 2) ?> <span style="font-size:0.7rem; color:#64748b;"><?= $currency ?></span></p></div></div>
        <div class="kpi-card" style="border-bottom: 3px solid #dc2626;"><div class="kpi-icon" style="background:#fef2f2; color:#dc2626;"><i class="ph-duotone ph-hourglass-high"></i></div><div class="kpi-info"><h4 style="color:#dc2626;"><?= $t['stat_rem'] ?></h4><p><?= number_format((float)($stats->total_remaining ?? 0), 2) ?> <span style="font-size:0.7rem; color:#64748b;"><?= $currency ?></span></p></div></div>
    </div>
    ...
    <form action="/ERP/projects/invoices" method="GET" class="search-bar">
        ...
        <select name="project_id" class="form-control" style="flex:1; min-width:160px;">
            ...
            <?php foreach($projects as $proj): ?>
                <option value="<?= $proj->id ?>" <?= ($projectId == $proj->id) ? 'selected' : '' ?>><?= htmlspecialchars($proj->name_ar) ?></option>
            <?php endforeach; ?>
        </select>
        ...
        <?php if(!empty($search) || !empty($projectId) || !empty($typeFilter) || !empty($statusFilter)): ?>
            <a href="/ERP/projects/invoices" class="btn-clear"><i class="ph-bold ph-x"></i> <?= $t['clear'] ?></a>
        <?php endif; ?>
    </form>
            ...
            <table class="pi-table">
                <thead>
                    <tr>
                        ...
                        <th style="width: 22%;"><?= $t['col_proj'] ?></th>
                        <th style="width: 12%;"><?= $t['col_type'] ?></th>
                        <th style="width: 12%;"><?= $t['col_date'] ?></th>
                        <th style="width: 14%; text-align: end;"><?= $t['col_net'] ?></th>
                        <th style="width: 14%;"><?= $t['col_paid'] ?></th>
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                <div style="font-weight: 800; color: var(--c-text-dark);"><?= htmlspecialchars($inv->project_name ?? '---') ?></div>
                                <div style="font-size: 0.78rem; color: var(--c-text-muted);"><i class="ph-bold ph-user"></i> <?= htmlspecialchars($inv->customer_name ?? '---') ?></div>
                            </td>
                            <td>
                                <span style="padding: 2px 8px; border-radius: 6px; font-weight: 800; font-size: 0.7rem; background:<?= $tp['bg'] ?>; color:<?= $tp['color'] ?>;">
                                    <?= $tp['label'] ?>
                                </span>
                            </td>
                            ...
                            <td style="text-align: end; font-family: monospace; font-weight: 900; color: var(--c-pi-dark); font-size: 0.95rem;">
                                <?= number_format($net, 2) ?> <?= $currency ?>
                            </td>
                            <td>
                                <div style="font-size:0.8rem; font-weight:800; color:#059669; font-family:monospace;"><?= $t['paid'] ?> <?= number_format($paid, 2) ?></div>
                                <div style="font-size:0.75rem; font-weight:800; color:<?= $rem > 0 ? '#dc2626' : '#64748b' ?>; font-family:monospace;"><?= $t['rem'] ?> <?= number_format($rem, 2) ?></div>
                            </td>
                            ...
                            <td style="text-align: center; white-space: nowrap;">
                                <a href="/ERP/projects/invoices/<?= $inv->id ?>" class="action-btn" title="عرض وطباعة"><i class="ph-bold ph-printer"></i></a>
                                <a href="/ERP/projects/invoices/<?= $inv->id ?>/edit" class="action-btn" title="تعديل"><i class="ph-bold ph-pencil-simple"></i></a>
                                <form action="/ERP/projects/invoices/<?= $inv->id ?>/delete" method="POST" style="display:inline;" onsubmit="return confirm('<?= $t['confirm_del'] ?>');">
    ...
    <?php if (isset($totalPages) && $totalPages > 1): ?>
        <div style="display:flex; justify-content:center; gap:8px; margin-top:24px;">
            <?php for($i = 1; $i <= $totalPages; $i++): ?>
                <a href="?page=<?= $i ?>&search=<?= urlencode($search) ?>&project_id=<?= urlencode($projectId) ?>&invoice_type=<?= urlencode($typeFilter) ?>&status=<?= urlencode($statusFilter) ?>" style="width: 36px; height: 36px; display: inline-flex; align-items: center; justify-content: center; border-radius: 8px; border: 1px solid var(--c-border); background: <?= $i == ($currentPage ?? 1) ? 'var(--c-pi)' : '#fff' ?>; color: <?= $i == ($currentPage ?? 1) ? '#fff' : 'var(--c-text-muted)' ?>; font-weight: 800; text-decoration:none;"><?= $i ?></a>
            <?php endfor; ?>
        </div>
    <?php endif; ?>
</div>