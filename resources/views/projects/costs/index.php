...
$t = [
    'ar' => [
        'title' => 'تكاليف ومصروفات الموقع', 'desc' => 'تسجيل وحصر مصاريف المواد، العمالة، المعدات والمقاولين الفرعيين بالمواقع.',
        'add_btn' => 'تسجيل مصروف موقع', 'col_num' => 'رقم السند', 'col_proj' => 'المشروع والتبويب',
        'col_dates' => 'التاريخ / المرجع', 'col_sup' => 'المورد / الحساب', 'col_val' => 'المبلغ المصروف',
        'col_status' => 'السداد', 'col_actions' => 'إجراءات', 'empty' => 'لا توجد مصروفات موقع تطابق بحثك.',
        'search' => 'ابحث برقم السند، البيان، المرجع، المورد...', 'btn_search' => 'بحث', 'clear' => 'إلغاء',
        'stat_total' => 'إجمالي السندات', 'stat_amt' => 'إجمالي المصروفات', 'stat_mat' => 'تكلفة المواد والتوريد', 'stat_eq' => 'معدات ومقاولين فرعيين',
        'cat_mat' => 'مواد وتوريدات', 'cat_lab' => 'عمالة وأجور', 'cat_eq' => 'معدات وآليات', 'cat_sub' => 'مقاولين فرعيين', 'cat_over' => 'مصروفات إدارية', 'cat_oth' => 'أخرى',
        'st_paid' => 'مسدد بالكامل', 'st_part' => 'مسدد جزئياً', 'st_unpaid' => 'غير مسدد',
        'confirm_del' => 'هل أنت متأكد من الحذف؟ سيعاد حساب التكلفة للمشروع.'
    ],
    'en' => [
        'title' => 'Site Expenses & Costs', 'desc' => 'Log and track materials, labor, equipment, and subcontractor site costs.',
        'add_btn' => 'Log Site Expense', 'col_num' => 'Voucher No.', 'col_proj' => 'Project & Category',
        'col_dates' => 'Date / Ref', 'col_sup' => 'Supplier / Account', 'col_val' => 'Amount Spent',
        'col_status' => 'Payment', 'col_actions' => 'Actions', 'empty' => 'No site expenses found.',
        'search' => 'Search by voucher, ref, desc, supplier...', 'btn_search' => 'Search', 'clear' => 'Clear',
        'stat_total' => 'Total Vouchers', 'stat_amt' => 'Total Expenses', 'stat_mat' => 'Materials Cost', 'stat_eq' => 'Equip & Subcontractors',
        'cat_mat' => 'Materials', 'cat_lab' => 'Labor', 'cat_eq' => 'Equipment', 'cat_sub' => 'Subcontractors', 'cat_over' => 'Overhead', 'cat_oth' => 'Other',
        'st_paid' => 'Fully Paid', 'st_part' => 'Partially Paid', 'st_unpaid' => 'Unpaid',
        'confirm_del' => 'Confirm delete? Project spent cost will be recalculated.'
    ]
][$isRtl ? 'ar' : 'en'];

$categoryMap = [
    'materials' => ['label' => $t['cat_mat'], 'color' => '#ea580c', 'bg' => '#ffedd5', 'icon' => 'ph-package'],
    'labor' => ['label' => $t['cat_lab'], 'color' => '#2563eb', 'bg' => '#eff6ff', 'icon' => 'ph-users-three'],
    'equipment' => ['label' => $t['cat_eq'], 'color' => '#d97706', 'bg' => '#fef3c7', 'icon' => 'ph-truck'],
    'subcontractor' => ['label' => $t['cat_sub'], 'color' => '#8b5cf6', 'bg' => '#f5f3ff', 'icon' => 'ph-handshake'],
    'overhead' => ['label' => $t['cat_over'], 'color' => '#0284c7', 'bg' => '#e0f2fe', 'icon' => 'ph-receipt'],
    'other' => ['label' => $t['cat_oth'], 'color' => '#64748b', 'bg' => '#f1f5f9', 'icon' => 'ph-dots-three-circle'],
];

$statusMap = [
    'paid' => ['label' => $t['st_paid'], 'color' => '#059669', 'bg' => '#ecfdf5'],
    'partially_paid' => ['label' => $t['st_part'], 'color' => '#d97706', 'bg' => '#fef3c7'],
    'unpaid' => ['label' => $t['st_unpaid'], 'color' => '#dc2626', 'bg' => '#fef2f2'],
];
?>

<style>
    :root { --c-pcost: #ea580c; --c-pcost-dark: #c2410c; --c-pcost-light: #ffedd5; --c-border: #cbd5e1; --c-text-dark: #0f172a; --c-text-muted: #64748b; }
    ...
    .btn-pcost { background: linear-gradient(135deg, var(--c-pcost), var(--c-pcost-dark)); color: #ffffff !important; border: none; padding: 10px 24px; border-radius: 10px; font-weight: 800; display: inline-flex; align-items: center; gap: 8px; text-decoration: none; box-shadow: 0 4px 12px rgba(234, 88, 12, 0.25); transition: 0.2s;}
    .btn-pcost:hover { transform: translateY(-2px); box-shadow: 0 6px 16px rgba(234, 88, 12, 0.35); }
    ...
    .form-control:focus { outline: none; border-color: var(--c-pcost); }
    ...
    .pcost-table tr:hover td { background: #f8fafc; }
...
<div class="pcost-wrapper" dir="<?= $isRtl ? 'rtl' : 'ltr' ?>">
    <div class="pcost-header">
        <div class="pcost-title-box">
            ...
            <div>
                <h2 class="pcost-title"><?= $t['title'] ?></h2>
                <p style="margin:4px 0 0 0; color:var(--c-text-muted);"><?= $t['desc'] ?></p>
            </div>
        </div>
        <a href="/ERP/projects/costs/create" class="btn-pcost"><i class="ph-bold ph-plus"></i> <?= $t['add_btn'] ?></a>
    </div>
    ...
    <div class="kpi-row">
        <div class="kpi-card"><div class="kpi-icon"><i class="ph-duotone ph-receipt"></i></div><div class="kpi-info"><h4><?= $t['stat_total'] ?></h4><p><?= number_format($stats->total_vouchers ?? 0) ?></p></div></div>
        <div class="kpi-card" style="border-bottom: 3px solid var(--c-pcost);"><div class="kpi-icon" style="background:var(--c-pcost-light); color:var(--c-pcost);"><i class="ph-duotone ph-coins"></i></div><div class="kpi-info"><h4 style="color:var(--c-pcost);"><?= $t['stat_amt'] ?></h4><p><?= number_format((float)($stats->total_amount ?? 0), 2) ?> <span style="font-size:0.7rem; color:#64748b;"><?= $currency ?></span></p></div></div>
        <div class="kpi-card" style="border-bottom: 3px solid #2563eb;"><div class="kpi-icon" style="background:#eff6ff; color:#2563eb;"><i class="ph-duotone ph-package"></i></div><div class="kpi-info"><h4 style="color:#2563eb;"><?= $t['stat_mat'] ?></h4><p><?= number_format((float)($stats->materials_cost ?? 0), 2) ?> <span style="font-size:0.7rem; color:#64748b;"><?= $currency ?></span></p></div></div>
        <div class="kpi-card" style="border-bottom: 3px solid #d97706;"><div class="kpi-icon" style="background:#fef3c7; color:#d97706;"><i class="ph-duotone ph-truck"></i></div><div class="kpi-info"><h4 style="color:#d97706;"><?= $t['stat_eq'] ?></h4><p><?= number_format((float)($stats->equipment_cost ?? 0), 2) ?> <span style="font-size:0.7rem; color:#64748b;"><?= $currency ?></span></p></div></div>
    </div>
    ...
    <form action="/ERP/projects/costs" method="GET" class="search-bar">
        ...
        <select name="cost_category" class="form-control" style="flex:1; min-width:140px;">
            ...
            <?php foreach($categoryMap as $k => $v): ?>
                <option value="<?= $k ?>" <?= ($categoryFilter===$k)?'selected':'' ?>><?= $v['label'] ?></option>
            <?php endforeach; ?>
        </select>
        ...
        <?php if(!empty($search) || !empty($projectId) || !empty($categoryFilter) || !empty($statusFilter)): ?>
            <a href="/ERP/projects/costs" class="btn-clear"><i class="ph-bold ph-x"></i> <?= $t['clear'] ?></a>
        <?php endif; ?>
    </form>
            ...
            <table class="pcost-table">
                <thead>
                    <tr>
                        ...
                        <th style="width: 18%;"><?= $t['col_sup'] ?></th>
                        <th style="width: 13%;"><?= $t['col_val'] ?></th>
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                <div style="font-weight: 800; color: var(--c-text-dark);"><?= htmlspecialchars($c->project_name ?? '---') ?></div>
                            ...
                            <td>
                                <div style="font-weight: 800; color: var(--c-text-dark);"><?= htmlspecialchars($c->supplier_name ?: ($c->account_name ?? '---')) ?></div>
                            ...
                            <td style="font-family: monospace; font-weight: 900; color: #dc2626; font-size: 0.98rem;">
                                <?= number_format((float)$c->amount, 2) ?> <span style="font-size:0.7rem; color:#64748b;"><?= $currency ?></span>
                            </td>
                            ...
                            <td style="text-align: center; white-space: nowrap;">
                                <a href="/ERP/projects/costs/<?= $c->id ?>" class="action-btn" title="عرض وطباعة السند"><i class="ph-bold ph-printer"></i></a>
                                <a href="/ERP/projects/costs/<?= $c->id ?>/edit" class="action-btn" title="تعديل"><i class="ph-bold ph-pencil-simple"></i></a>
                                <form action="/ERP/projects/costs/<?= $c->id ?>/delete" method="POST" style="display:inline;" onsubmit="return confirm('<?= $t['confirm_del'] ?>');">
    ...
    <?php if (isset($totalPages) && $totalPages > 1): ?>
        <div style="display:flex; justify-content:center; gap:8px; margin-top:24px;">
            <?php for($i = 1; $i <= $totalPages; $i++): ?>
                <a href="?page=<?= $i ?>&search=<?= urlencode($search) ?>&project_id=<?= urlencode($projectId) ?>&cost_category=<?= urlencode($categoryFilter) ?>&payment_status=<?= urlencode($statusFilter) ?>" style="width: 36px; height: 36px; display: inline-flex; align-items: center; justify-content: center; border-radius: 8px; border: 1px solid var(--c-border); background: <?= $i == ($currentPage ?? 1) ? 'var(--c-pcost)' : '#fff' ?>; color: <?= $i == ($currentPage ?? 1) ? '#fff' : 'var(--c-text-muted)' ?>; font-weight: 800; text-decoration:none;"><?= $i ?></a>
            <?php endfor; ?>
        </div>
    <?php endif; ?>
</div>