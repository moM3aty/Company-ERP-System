...
function getReturnBadge($status) {
    $map = [
        ...
        'approved' => ['bg' => '#eff6ff', 'color' => '#2563eb', 'label' => 'موافق عليه'],
        'completed' => ['bg' => '#fef2f2', 'color' => '#dc2626', 'label' => 'تم الإرجاع والخصم'],
        'cancelled' => ['bg' => '#f1f5f9', 'color' => '#94a3b8', 'label' => 'ملغى']
    ];
...
<div class="mod-wrapper" dir="<?= $isRtl ? 'rtl' : 'ltr' ?>">
    ...
    <div class="kpi-row">
        <div class="kpi-card">
            ...
            <div class="kpi-info"><h4>عدد المرتجعات المبحوثة</h4><p><?= count($returns ?? []) ?></p></div>
        </div>
        <div class="kpi-card" style="border-bottom: 3px solid var(--c-red);">
            <div class="kpi-icon"><i class="ph-duotone ph-money"></i></div>
            <div class="kpi-info"><h4 style="color:var(--c-red);">إجمالي قيمة المرتجعات</h4><p><?= number_format($totalReturnSum, 2) ?></p></div>
        </div>
    </div>

    <!-- Search Form -->
    <form action="/ERP/purchasing/returns" method="GET" class="search-bar">
        <input type="text" name="search" class="search-input" placeholder="ابحث برقم المرتجع، رقم الفاتورة، المورد، أو سبب الإرجاع..." value="<?= htmlspecialchars($search ?? '') ?>">
        <button type="submit" class="btn-search"><i class="ph-bold ph-magnifying-glass"></i> بحث</button>
        <?php if(!empty($search)): ?>
            <a href="/ERP/purchasing/returns" class="btn-clear"><i class="ph-bold ph-x"></i> إلغاء</a>
        <?php endif; ?>
    </form>
                    ...
                    <tr>
                        <td>
                            ...
                            <?php if(!empty($ret->invoice_number)): ?>
                                <div style="color: var(--c-text-muted); font-size: 0.8rem; margin-top:2px;">الفاتورة: <span style="font-family:monospace; font-weight:bold; color:#0f172a;"><?= htmlspecialchars($ret->invoice_number) ?></span></div>
                            <?php endif; ?>
                        </td>
                        <td>
                            ...
                            <?php if(!empty($ret->reason)): ?>
                                <div style="color: var(--c-text-muted); font-size: 0.8rem; margin-top:2px;">السبب: <span><?= htmlspecialchars($ret->reason) ?></span></div>
                            <?php endif; ?>
                        </td>
                        <td>
                            <div style="font-weight: 700; color: #334155;"><i class="ph-bold ph-calendar-blank"></i> <?= $ret->return_date ?></div>
                            <div style="color: var(--c-text-muted); font-size: 0.8rem; margin-top:2px;">البنود: <?= $ret->items_count ?></div>
                        </td>
                        ...
                        <td style="text-align: center; white-space: nowrap;">
                            ...
                            <form action="/ERP/purchasing/returns/<?= $ret->id ?>/delete" method="POST" style="display:inline;" onsubmit="return confirm('تأكيد الحذف؟');">
