...
<style>
    ...
    @media print {
        ...
        .ret-wrapper { max-width: 100% !important; margin: 0 !important; padding: 0 !important; }
...
<div class="ret-wrapper" dir="<?= $dir ?>">
    <div class="top-bar">
        ...
        <button onclick="window.print()" class="btn-act"><i class="ph-bold ph-printer"></i> طباعة إشعار الخصم</button>
    </div>
    ...
    <div class="print-canvas">
        <div class="c-header">
            <div>
                ...
                <p style="margin:4px 0 0 0; color:#64748b; font-weight:700;">إذن مرتجع مشتريات - إشعار خصم (Debit Note)</p>
            </div>
            <div style="text-align: <?= $isAr ? 'left' : 'right' ?>;">
                ...
                <div style="margin-top:4px; font-weight:800; color: #0f172a; text-transform: uppercase;">تاريخ الإرجاع: <?= $returnOrder->return_date ?></div>
            </div>
        </div>

        <div class="info-grid">
            <div class="info-box">
                <h5>إلى المورد (Supplier)</h5>
                <p style="font-size: 1.1rem; color: var(--c-red);"><i class="ph-fill ph-buildings"></i> <?= htmlspecialchars($returnOrder->supplier_name ?? '---') ?></p>
                <p><span style="color:#64748b;">كود المورد:</span> <span style="font-family:monospace;"><?= htmlspecialchars($returnOrder->supplier_code ?? '---') ?></span></p>
                <p><span style="color:#64748b;">الهاتف:</span> <?= htmlspecialchars($returnOrder->supplier_phone ?? '---') ?></p>
            </div>
            <div class="info-box">
                <h5>بيانات الفاتورة والسبب</h5>
                <p><span style="color:#64748b;">الفاتورة المرتبطة:</span> <strong style="font-family:monospace;"><?= htmlspecialchars($returnOrder->invoice_number ?? 'بدون فاتورة') ?></strong></p>
                <p><span style="color:#64748b;">سبب الإرجاع:</span> <?= htmlspecialchars($returnOrder->reason ?? 'غير محدد') ?></p>
                <p><span style="color:#64748b;">الحالة:</span> <strong style="color:var(--c-red);"><?= strtoupper($returnOrder->status) ?></strong></p>
            </div>
        </div>
        ...
        <div>
            <?php if(!empty($returnOrder->notes)): ?>
                <div style="float: <?= $isAr ? 'right' : 'left' ?>; width: 50%; color: #334155; font-size: 0.9rem; font-weight: 600; padding: 10px; border: 1px dashed #cbd5e1; border-radius: 8px;">
                    <strong style="color: var(--c-red);">ملاحظات إضافية:</strong> <br><?= nl2br(htmlspecialchars($returnOrder->notes)) ?>
                </div>
            <?php endif; ?>
            ...
            <div class="totals-area">
                ...
                <div class="totals-row grand">
                    <span>صافي قيمة الخصم:</span>
                    <span style="font-family: monospace;"><?= number_format($returnOrder->total_amount, 2) ?></span>
                </div>
            </div>
        ...
        <div style="margin-top: 60px; display: flex; justify-content: space-between; text-align: center; font-weight: 800; font-size: 0.9rem; color: #0f172a;">
            <div style="flex: 1;">
                <div>توقيع أمين المخزن المسلّم</div>
                <div style="border-top: 1px dashed #cbd5e1; width: 60%; margin: 40px auto 0 auto;"></div>
            </div>
            <div style="flex: 1;">
                <div>إعداد إدارة المشتريات</div>
                <div style="border-top: 1px dashed #cbd5e1; width: 60%; margin: 40px auto 0 auto;"></div>
            </div>
            <div style="flex: 1;">
                <div>توقيع واستلام مندوب المورد</div>
                <div style="border-top: 1px dashed #cbd5e1; width: 60%; margin: 40px auto 0 auto;"></div>
            </div>
        </div>
    </div>
