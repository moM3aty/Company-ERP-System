...
$t = [
    'ar' => [
        ...
        'confirm_delete' => 'تأكيد الحذف نهائياً؟'
    ],
    'en' => [
...
<style>
    ...
    .action-btn { width: 36px; height: 36px; border-radius: 10px; border: 1px solid var(--border); background: var(--surface); color: var(--text-muted); display: inline-flex; align-items: center; justify-content: center; cursor: pointer; transition: 0.2s; text-decoration: none; font-size: 1.1rem; margin: 0 3px; }
...
<div class="premium-wrapper" dir="<?= $isRtl ? 'rtl' : 'ltr' ?>">
    ...
    <form action="/ERP/purchasing/suppliers" method="GET" class="search-bar">
        ...
        <?php if(!empty($search)): ?>
            <a href="/ERP/purchasing/suppliers" class="btn-clear"><i class="ph-bold ph-x"></i> <?= $t['clear'] ?></a>
        <?php endif; ?>
    </form>
                    ...
                    <?php else: ?>
                        ...
                        ?>
                            <tr>
                                ...
                                <td style="text-align: end; font-family: monospace; font-weight: 800; color: var(--primary);">
                                    <?= number_format($sup->credit_limit, 2) ?> <?= $currency ?>
                                </td>
                                ...
                                <td style="text-align: center; white-space: nowrap;">
                                    <a href="/ERP/purchasing/suppliers/<?= $sup->id ?>" class="action-btn" title="الملف والطباعة"><i class="ph-bold ph-eye"></i></a>
                                    <a href="/ERP/purchasing/suppliers/<?= $sup->id ?>/edit" class="action-btn" title="تعديل"><i class="ph-bold ph-pencil-simple"></i></a>
                                    <form action="/ERP/purchasing/suppliers/<?= $sup->id ?>/delete" method="POST" style="display:inline;" onsubmit="return confirm('<?= $t['confirm_delete'] ?>');">
