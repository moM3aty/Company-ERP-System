...
$t = [
    'ar' => [
        ...
        'add_btn' => 'فاتورة جديدة', 'col_num' => 'رقم الفاتورة / أمر الشراء', 'col_sup' => 'المورد / فاتورة المورد',
        'col_dates' => 'الإصدار / الاستحقاق', 'col_val' => 'المبلغ الإجمالي / المتبقي', 'col_status' => 'الحالة', 'col_actions' => 'إجراءات', 'empty' => 'لا توجد فواتير مشتريات تطابق بحثك.'
    ],
    'en' => [
        'title' => 'Purchase Invoices (Bills)', 'desc' => 'Manage supplier bills, accounts payable, and track due payments.',
        'add_btn' => 'New Invoice', 'col_num' => 'Invoice / PO Number', 'col_sup' => 'Supplier / Vendor Bill No.',
        'col_dates' => 'Issue / Due Date', 'col_val' => 'Total / Balance Due', 'col_status' => 'Status', 'col_actions' => 'Actions', 'empty' => 'No purchase invoices found.'
    ]
][$isRtl ? 'ar' : 'en'];
...
function getInvoiceBadge($status) {
    $map = [
        ...
        'unpaid' => ['bg' => '#fef2f2', 'color' => '#dc2626', 'label' => 'غير مدفوعة'],
        'partially_paid' => ['bg' => '#fef3c7', 'color' => '#d97706', 'label' => 'مدفوعة جزئياً'],
        'paid' => ['bg' => '#ecfdf5', 'color' => '#059669', 'label' => 'مدفوعة بالكامل'],
        'cancelled' => ['bg' => '#f1f5f9', 'color' => '#94a3b8', 'label' => 'ملغاة']
    ];
...
if (!empty($invoices)) {
    foreach ($invoices as $inv) {
        $totalAmountSum += $inv->total_amount;
        if ($inv->status !== 'paid' && $inv->status !== 'cancelled') {
            $totalUnpaid += ($inv->total_amount - $inv->paid_amount);
        }
    }
}
?>
...
<style>
    ...
    .search-input:focus { border-color: var(--c-rose); background: #ffffff; outline: none; box-shadow: 0 0 0 3px var(--c-rose-light); }
...
<div class="mod-wrapper" dir="<?= $isRtl ? 'rtl' : 'ltr' ?>">
    ...
    <div class="kpi-row">
        <div class="kpi-card">
            ...
            <div class="kpi-info"><h4>إجمالي الفواتير</h4><p><?= count($invoices ?? []) ?></p></div>
        </div>
        <div class="kpi-card" style="border-bottom: 3px solid var(--c-rose);">
            <div class="kpi-icon"><i class="ph-duotone ph-coins"></i></div>
            <div class="kpi-info"><h4 style="color:var(--c-rose);">قيمة فواتير الصفحة</h4><p><?= number_format($totalAmountSum, 2) ?></p></div>
        </div>
        <div class="kpi-card" style="border-bottom: 3px solid #dc2626;">
            ...
            <div class="kpi-info"><h4 style="color:#dc2626;">مستحقات غبر مدفوعة</h4><p><?= number_format($totalUnpaid, 2) ?></p></div>
        </div>
    </div>

    <!-- Search Form -->
    <form action="/ERP/purchasing/invoices" method="GET" class="search-bar">
        <input type="text" name="search" class="search-input" placeholder="ابحث برقم الفاتورة، رقم فاتورة المورد، المورد أو أمر الشراء..." value="<?= htmlspecialchars($search ?? '') ?>">
        <button type="submit" class="btn-search"><i class="ph-bold ph-magnifying-glass"></i> بحث</button>
        <?php if(!empty($search)): ?>
            <a href="/ERP/purchasing/invoices" class="btn-clear"><i class="ph-bold ph-x"></i> إلغاء</a>
        <?php endif; ?>
    </form>
                    ...
                    <tr>
                        <td>
                            ...
                            <?php if(!empty($inv->po_number)): ?>
                                <div style="color: var(--c-text-muted); font-size: 0.8rem; margin-top:2px;">أمر شراء: <span style="font-family:monospace; font-weight:bold; color:#0f172a;"><?= htmlspecialchars($inv->po_number) ?></span></div>
                            <?php endif; ?>
                        </td>
                        <td>
                            ...
                            <?php if(!empty($inv->supplier_invoice_number)): ?>
                                <div style="color: var(--c-text-muted); font-size: 0.8rem; margin-top:2px;">فاتورة المورد: <span style="font-family:monospace;"><?= htmlspecialchars($inv->supplier_invoice_number) ?></span></div>
                            <?php endif; ?>
                        </td>
                        <td>
                            <div style="font-weight: 600; color: #475569; font-size: 0.85rem;"><i class="ph-bold ph-calendar-plus"></i> إصدار: <?= $inv->invoice_date ?></div>
                            <div style="font-weight: 600; color: #dc2626; font-size: 0.85rem; margin-top:2px;"><i class="ph-bold ph-calendar-check"></i> استحقاق: <?= $inv->due_date ?></div>
                        </td>
                        <td style="text-align: end;">
                            <div style="font-family: monospace; font-weight: 900; color: var(--c-text-dark); font-size: 1.05rem;"><?= number_format($inv->total_amount, 2) ?></div>
                            <?php if($due > 0 && $inv->status !== 'paid'): ?>
                                <div style="font-family: monospace; font-size: 0.8rem; color: #dc2626; font-weight: bold; margin-top:2px;">متبقي: <?= number_format($due, 2) ?></div>
                            <?php endif; ?>
                        </td>
                        ...
                        <td style="text-align: center; white-space: nowrap;">
                            <a href="/ERP/purchasing/invoices/<?= $inv->id ?>" class="action-btn" title="معاينة وطباعة"><i class="ph-bold ph-printer"></i></a>
                            <a href="/ERP/purchasing/invoices/<?= $inv->id ?>/edit" class="action-btn" title="تعديل"><i class="ph-bold ph-pencil-simple"></i></a>
                            <form action="/ERP/purchasing/invoices/<?= $inv->id ?>/delete" method="POST" style="display:inline;" onsubmit="return confirm('تأكيد الحذف؟');">
