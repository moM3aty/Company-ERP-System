...
$retId         = (int)($ret->id ?? 0);
$returnNumber  = htmlspecialchars((string)($ret->return_number ?? '---'));
$returnDate    = htmlspecialchars((string)($ret->return_date ?? date('Y-m-d')));
$partyName     = htmlspecialchars((string)($ret->party_name ?? '---'));
$warehouseName = htmlspecialchars((string)($ret->warehouse_name ?? '---'));
$warehouseCode = htmlspecialchars((string)($ret->warehouse_code ?? '---'));
$notes         = htmlspecialchars((string)($ret->notes ?? ''));
$status        = (string)($ret->status ?? 'draft');
$type          = (string)($ret->return_type ?? 'sales_return');
...
$t = [
    'ar' => [
        'print' => 'طباعة إذن المرتجع',
        'sub_sales' => 'إذن مرتجع مخزني - مرتجع مبيعات',
        'sub_purchase' => 'إذن مرتجع مخزني - مرتجع مشتريات',
        'date_label' => 'التاريخ:',
        'party_sales' => 'الطرف المعني (العميل)',
        'party_purchase' => 'الطرف المعني (المورد)',
        'wh_title' => 'المستودع المعني',
        'notes_title' => 'البيان العام / سبب الإرجاع:',
        'col_code' => 'كود الصنف', 'col_name' => 'اسم ووصف الصنف',
        'col_unit' => 'الوحدة', 'col_qty' => 'الكمية', 'col_reason' => 'السبب المحدد',
        'no_items' => 'لا توجد أصناف مسجلة لهذا الإذن.',
        'tot_qty' => 'إجمالي الكميات (Total Quantity):',
        'sig_inspector' => 'مسؤول الفحص / المعاينة',
        'sig_store' => 'أمين المخزن',
        'sig_party' => 'الجهة المسلمة / المستلمة',
        'sig_sub' => 'الاسم والتوقيع والختم'
    ],
    'en' => [
        'print' => 'Print Return Note',
        'sub_sales' => 'Stock Return Note - Sales Return',
        'sub_purchase' => 'Stock Return Note - Purchase Return',
        'date_label' => 'Date:',
        'party_sales' => 'Party Name (Customer)',
        'party_purchase' => 'Party Name (Supplier)',
        'wh_title' => 'Warehouse',
        'notes_title' => 'General Notes / Return Reason:',
        'col_code' => 'Item Code', 'col_name' => 'Item Name & Description',
        'col_unit' => 'Unit', 'col_qty' => 'Quantity', 'col_reason' => 'Specific Reason',
        'no_items' => 'No items registered for this return note.',
        'tot_qty' => 'Total Quantity:',
        'sig_inspector' => 'Inspection Officer',
        'sig_store' => 'Storekeeper',
        'sig_party' => 'Recipient / Delivering Party',
...
<div class="doc-wrapper" dir="<?= $dir ?>">
    ...
    <div class="print-canvas">
        <div class="c-header">
            <div>
                ...
                <p style="margin:4px 0 0 0; color:#64748b; font-weight:700;">
                    <?= $type === 'sales_return' ? $t['sub_sales'] : $t['sub_purchase'] ?>
                </p>
            </div>
            <div style="text-align: <?= $isAr ? 'left' : 'right' ?>;">
                ...
                <div style="margin-top:6px; font-weight:800; color: #0f172a;"><?= $t['date_label'] ?> <?= $returnDate ?></div>
        ...
        <div class="info-grid">
            <div class="info-card">
                <h5><?= $type === 'sales_return' ? $t['party_sales'] : $t['party_purchase'] ?></h5>
        ...
        <table class="table-print">
            <thead>
                <tr>
                    ...
                    <th style="width: 15%;"><?= $t['col_reason'] ?></th>
                </tr>
            </thead>
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td style="font-size: 0.85rem; color: #64748b;"><?= htmlspecialchars((string)($item->reason ?? '---')) ?></td>
                    </tr>
                <?php endforeach; else: ?>
                    <tr>
                        <td colspan="6" style="text-align: center; padding: 20px; color: #94a3b8;"><?= $t['no_items'] ?></td>
                    </tr>
                <?php endif; ?>
            </tbody>
            <tfoot>
