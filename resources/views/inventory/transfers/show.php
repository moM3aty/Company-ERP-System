...
// حماية من انعدام البيانات
$transferNumber = htmlspecialchars($transfer->transfer_number ?? '---');
$transferDate   = htmlspecialchars($transfer->transfer_date ?? date('Y-m-d'));
$fromWhName     = htmlspecialchars($transfer->from_wh_name ?? '---');
$fromWhCode     = htmlspecialchars($transfer->from_wh_code ?? '---');
$toWhName       = htmlspecialchars($transfer->to_wh_name ?? '---');
$toWhCode       = htmlspecialchars($transfer->to_wh_code ?? '---');
...
<div class="doc-wrapper" dir="<?= $dir ?>">
    <div class="top-bar">
        ...
        <button onclick="window.print()" class="btn-act"><i class="ph-bold ph-printer"></i> طباعة إذن التحويل</button>
    </div>
    ...
    <div class="print-canvas">
        <div class="c-header">
            <div>
                ...
                <p style="margin:4px 0 0 0; color:#64748b; font-weight:700;">إذن تحويل مخزني (Stock Transfer Note)</p>
            </div>
            <div style="text-align: <?= $isAr ? 'left' : 'right' ?>;">
                ...
                <div style="margin-top:6px; font-weight:800; color: #0f172a;">التاريخ: <?= $transferDate ?></div>
        ...
        <div class="route-box">
            <div class="wh-node">
                <h5><i class="ph-fill ph-export" style="color:#ea580c;"></i> منصرف من مستودع</h5>
                <h3><?= $fromWhName ?></h3>
                <p><?= $fromWhCode ?></p>
            </div>
            ...
            <div class="wh-node">
                <h5><i class="ph-fill ph-download-simple" style="color:#059669;"></i> وارد إلى مستودع</h5>
                <h3><?= $toWhName ?></h3>
                <p><?= $toWhCode ?></p>
            </div>
        </div>
        ...
        <?php if(!empty($notes)): ?>
            <div style="margin-bottom: 24px; padding: 12px 16px; border-right: 4px solid var(--c-amber); background: #fef3c7; color: #b45309; font-weight: 700; border-radius: 6px;">
                <strong style="color: #92400e;">البيان / بوليصة الشحن:</strong> <?= $notes ?>
            </div>
        <?php endif; ?>
        ...
        <table class="table-print">
            <thead>
                <tr>
                    ...
                    <th style="width: 22%;">كود الصنف</th>
                    <th style="width: 48%;">اسم ووصف الصنف</th>
                    <th style="width: 10%; text-align: center;">الوحدة</th>
                    <th style="width: 14%; text-align: center;">الكمية المحولة</th>
                </tr>
            </thead>
            <tbody>
                ...
                <?php endforeach; else: ?>
                    <tr>
                        <td colspan="5" style="text-align: center; padding: 20px; color: #94a3b8;">لا توجد أصناف مسجلة لهذا أمر التحويل.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
            <tfoot>
        ...
        <div class="signatures-grid">
            <div class="sig-box">
                <div>أمين المخزن (المنصرف)</div>
                <div style="color: #64748b; font-size: 0.75rem; margin-top: 4px;">الاسم والتوقيع</div>
                <div class="sig-line"></div>
            </div>
            <div class="sig-box">
                <div>السائق / الناقل</div>
                <div style="color: #64748b; font-size: 0.75rem; margin-top: 4px;">رقم السيارة والتوقيع</div>
                <div class="sig-line"></div>
            </div>
            <div class="sig-box">
                <div>أمين المخزن (المستلم)</div>
                <div style="color: #64748b; font-size: 0.75rem; margin-top: 4px;">الاسم، التوقيع والختم</div>
                <div class="sig-line"></div>
            </div>
        </div>
    </div>
