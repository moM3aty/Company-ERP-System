...
$adjNumber     = htmlspecialchars($adj->adjustment_number ?? '---');
$adjDate       = htmlspecialchars($adj->adjustment_date ?? date('Y-m-d'));
$warehouseName = htmlspecialchars($adj->warehouse_name ?? '---');
$warehouseCode = htmlspecialchars($adj->warehouse_code ?? '---');
$reason        = htmlspecialchars($adj->reason ?? '---');
$notes         = htmlspecialchars($adj->notes ?? '');
$status        = $adj->status ?? 'draft';
$type          = $adj->adjustment_type ?? 'addition';
...
<div class="doc-wrapper" dir="<?= $dir ?>">
    <div class="top-bar">
        ...
        <button onclick="window.print()" class="btn-act"><i class="ph-bold ph-printer"></i> طباعة إذن التسوية</button>
    </div>
    ...
    <div class="print-canvas">
        <div class="c-header">
            <div>
                ...
                <p style="margin:4px 0 0 0; color:#64748b; font-weight:700;">
                    إذن تسوية مخزنية - <?= $type === 'addition' ? 'إضافة (فائض جرد)' : 'خصم (عجز / تلف)' ?>
                </p>
            </div>
            <div style="text-align: <?= $isAr ? 'left' : 'right' ?>;">
                ...
                <div style="margin-top:6px; font-weight:800; color: #0f172a;">التاريخ: <?= $adjDate ?></div>
        ...
        <?php if(!empty($notes)): ?>
            <div style="margin-bottom: 24px; padding: 12px 16px; border-right: 4px solid var(--c-amber); background: #fef3c7; color: #b45309; font-weight: 700; border-radius: 6px;">
                <strong style="color: #92400e;">الملاحظات التفصيلية:</strong> <?= $notes ?>
            </div>
        <?php endif; ?>
        ...
        <table class="table-print">
            <thead>
                <tr>
                    ...
                    <th style="width: 44%;">اسم ووصف الصنف</th>
                    <th style="width: 10%; text-align: center;">الوحدة</th>
                    <th style="width: 18%; text-align: center;">الكمية المعدلة</th>
                </tr>
            </thead>
            <tbody>
        ...
        <div class="signatures-grid">
            ...
            <div class="sig-box">
                <div>مدير إدارة المخازن / الاعتماد</div>
                <div style="color: #64748b; font-size: 0.75rem; margin-top: 4px;">التوقيع والختم</div>
                <div class="sig-line"></div>
            </div>
        </div>
    </div>
