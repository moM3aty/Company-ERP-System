...
$t = [
    'ar' => [
        'title' => 'التسويات المخزنية (Stock Adjustments)', 'desc' => 'معالجة وتوثيق نتائج الجرد الدوري، تسوية العجز، الزيادة والأصناف التالفة.',
        'add_btn' => 'إذن تسوية جديد', 'col_num' => 'رقم التسوية', 'col_type' => 'نوع التسوية',
        'col_wh' => 'المستودع', 'col_reason' => 'السبب / البيان', 'col_date' => 'التاريخ',
        'col_status' => 'الحالة', 'col_actions' => 'إجراءات', 'empty' => 'لا توجد تسويات مخزنية تطابق بحثك.',
        'search_ph' => 'ابحث برقم التسوية، السبب، أو المستودع...', 'btn_search' => 'بحث', 'btn_clear' => 'إلغاء',
        'kpi_total' => 'إجمالي التسويات', 'kpi_add' => 'تسويات إضافة (فائض)', 'kpi_sub' => 'تسويات خصم (عجز / تلف)',
        'active_scope' => 'الفرع النشط:', 'print_list' => 'طباعة القائمة',
        'confirm_delete' => 'هل أنت متأكد من حذف أمر التسوية؟'
    ],
    'en' => [
        'title' => 'Stock Adjustments', 'desc' => 'Manage inventory reconciliation, surplus, stock deficit, and damaged goods.',
        'add_btn' => 'New Adjustment Note', 'col_num' => 'Adjustment No.', 'col_type' => 'Adjustment Type',
        'col_wh' => 'Warehouse', 'col_reason' => 'Reason / Details', 'col_date' => 'Date',
        'col_status' => 'Status', 'col_actions' => 'Actions', 'empty' => 'No stock adjustments found.',
        'search_ph' => 'Search by adjustment number, reason, or warehouse...', 'btn_search' => 'Search', 'btn_clear' => 'Clear',
        'kpi_total' => 'Total Adjustments', 'kpi_add' => 'Additions (Surplus)', 'kpi_sub' => 'Deductions (Deficit)',
        'active_scope' => 'Active Branch:', 'print_list' => 'Print List',
        'confirm_delete' => 'Are you sure you want to delete this adjustment note?'
    ]
][$isRtl ? 'ar' : 'en'];

function getAdjustmentStatusBadge($status, $isRtl) {
    $map = [
...
function getAdjustmentTypeBadge($type, $isRtl) {
    if ($type === 'addition') {
        $lbl = $isRtl ? 'إضافة رصيد (تسوية فائض)' : 'Addition (Surplus)';
        return "<span class='type-badge' style='background:#ecfdf5; color:#059669; padding:4px 10px; border-radius:6px; font-weight:800; font-size:0.75rem;'><i class='ph-bold ph-plus-circle'></i> {$lbl}</span>";
    }
    $lbl = $isRtl ? 'خصم رصيد (تسوية عجز / تلف)' : 'Deduction (Deficit)';
    return "<span class='type-badge' style='background:#fef2f2; color:#dc2626; padding:4px 10px; border-radius:6px; font-weight:800; font-size:0.75rem;'><i class='ph-bold ph-minus-circle'></i> {$lbl}</span>";
}
?>
...
<div class="mod-wrapper" dir="<?= $isRtl ? 'rtl' : 'ltr' ?>">
    <div class="mod-header">
        ...
        <div style="display: flex; gap: 10px;">
            ...
            <a href="/ERP/inventory/adjustments/create" class="btn-primary"><i class="ph-bold ph-plus"></i> <?= $t['add_btn'] ?></a>
        </div>
    </div>
    ...
    <div class="kpi-row">
        ...
        <div class="kpi-card" style="border-bottom: 3px solid #059669;">
            ...
            <div class="kpi-info"><h4 style="color:#059669;"><?= $t['kpi_add'] ?></h4><p><?= number_format($stats->addition_count ?? 0) ?></p></div>
        </div>
        <div class="kpi-card" style="border-bottom: 3px solid #dc2626;">
            ...
            <div class="kpi-info"><h4 style="color:#dc2626;"><?= $t['kpi_sub'] ?></h4><p><?= number_format($stats->subtraction_count ?? 0) ?></p></div>
        </div>
    </div>
    ...
    <form action="/ERP/inventory/adjustments" method="GET" class="search-bar">
        ...
        <?php if(!empty($search)): ?>
            <a href="/ERP/inventory/adjustments" class="btn-clear"><i class="ph-bold ph-x"></i> <?= $t['btn_clear'] ?></a>
        <?php endif; ?>
    </form>
        ...
        <table class="mod-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 20%;"><?= $t['col_reason'] ?></th>
                    <th style="width: 10%;"><?= $t['col_date'] ?></th>
                    <th style="width: 8%; text-align: center;"><?= $t['col_status'] ?></th>
                    <th style="width: 7%; text-align: center;"><?= $t['col_actions'] ?></th>
                </tr>
            </thead>
            <tbody>
                ...
                <?php else: foreach ($adjustments as $a): ?>
                    <tr>
                        <td>
                            <div style="font-weight: 900; color: var(--c-amber); font-family: monospace; font-size: 1.05rem;"><i class="ph-bold ph-hash"></i> <?= htmlspecialchars((string)$a->adjustment_number) ?></div>
                            <div style="color: var(--c-text-muted); font-size: 0.8rem; margin-top:2px;">أصناف: <strong><?= (int)($a->items_count ?? 0) ?></strong></div>
                        </td>
                        <td><?= getAdjustmentTypeBadge($a->adjustment_type ?? 'addition', $isRtl) ?></td>
                        <td>
                            <div style="font-weight: 700; color: #334155;"><i class="ph-fill ph-warehouse text-slate-400"></i> <?= htmlspecialchars((string)($a->warehouse_name ?? '---')) ?></div>
                        </td>
                        <td>
                            <div style="font-weight: 600; color: #475569; font-size:0.85rem;"><?= htmlspecialchars((string)($a->reason ?? '---')) ?></div>
                        </td>
                        <td><div style="font-weight: 700; color: #334155;"><i class="ph-bold ph-calendar-blank"></i> <?= htmlspecialchars((string)$a->adjustment_date) ?></div></td>
                        <td style="text-align: center;"><?= getAdjustmentStatusBadge($a->status ?? 'draft', $isRtl) ?></td>
                        
                        <td style="text-align: center; white-space: nowrap;">
                            <a href="/ERP/inventory/adjustments/<?= $a->id ?>" class="action-btn" title="معاينة وطباعة"><i class="ph-bold ph-printer"></i></a>
                            
                            <?php if(trim((string)$a->status) !== 'approved'): ?>
                                <a href="/ERP/inventory/adjustments/<?= $a->id ?>/edit" class="action-btn" title="تعديل"><i class="ph-bold ph-pencil-simple"></i></a>
                                <form action="/ERP/inventory/adjustments/<?= $a->id ?>/delete" method="POST" style="display:inline;" onsubmit="return confirm('<?= $t['confirm_delete'] ?>');">
