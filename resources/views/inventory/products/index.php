...
<div class="mod-wrapper" dir="<?= $isRtl ? 'rtl' : 'ltr' ?>">
    ...
    <div class="kpi-row">
        <div class="kpi-card">
            ...
            <div class="kpi-info"><h4>إجمالي الأصناف بالدليل</h4><p><?= number_format($stats->total ?? 0) ?></p></div>
        </div>
        <div class="kpi-card" style="border-bottom: 3px solid #059669;">
            ...
            <div class="kpi-info"><h4 style="color:#059669;">أصناف نشطة</h4><p><?= number_format($stats->active ?? 0) ?></p></div>
        </div>
        <div class="kpi-card" style="border-bottom: 3px solid #dc2626;">
            ...
            <div class="kpi-info"><h4 style="color:#dc2626;">أصناف متوقفة</h4><p><?= number_format($stats->inactive ?? 0) ?></p></div>
        </div>
    </div>

    <!-- Search Form -->
    <form action="/ERP/inventory/products" method="GET" class="search-bar">
        <input type="text" name="search" class="search-input" placeholder="ابحث بكود الصنف، الباركود، الاسم، أو الفئة..." value="<?= htmlspecialchars($search ?? '') ?>">
        <button type="submit" class="btn-search"><i class="ph-bold ph-magnifying-glass"></i> بحث</button>
        <?php if(!empty($search)): ?>
            <a href="/ERP/inventory/products" class="btn-clear"><i class="ph-bold ph-x"></i> إلغاء</a>
        <?php endif; ?>
    </form>
            ...
            <tbody>
                ...
                <?php else: foreach ($products as $p): ?>
                    <tr>
                        ...
                        <td>
                            <div style="font-weight: 700; color: #334155;"><i class="ph-fill ph-folder text-slate-400"></i> <?= htmlspecialchars($p->category_name ?? 'بدون فئة') ?></div>
                            <div style="color: var(--c-text-muted); font-size: 0.8rem; margin-top:2px;">الوحدة: <strong><?= htmlspecialchars($p->unit) ?></strong></div>
                        </td>
                        <td style="text-align: end;">
                            <div style="font-family: monospace; font-weight: 600; color: #64748b; font-size: 0.85rem;">شراء: <?= number_format($p->purchase_price, 2) ?></div>
                            <div style="font-family: monospace; font-weight: 900; color: #0f172a; font-size: 1.05rem; margin-top:2px;">بيع: <?= number_format($p->selling_price, 2) ?></div>
                        </td>
                        <td style="text-align: center;">
                            <?php if($p->is_active): ?> <span class="status-active">نشط</span>
                        ...
                        <td style="text-align: center; white-space: nowrap;">
                            ...
                            <form action="/ERP/inventory/products/<?= $p->id ?>/delete" method="POST" style="display:inline;" onsubmit="return confirm('هل أنت متأكد من حذف هذا الصنف؟');">
