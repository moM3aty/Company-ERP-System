...
$actionUrl = $isEdit ? "/ERP/inventory/delivery-notes/{$note->id}/update" : "/ERP/inventory/delivery-notes/store";
?>
...
<div class="form-wrapper" dir="<?= $isRtl ? 'rtl' : 'ltr' ?>">
    <div class="form-header">
        <a href="/ERP/inventory/delivery-notes" class="back-btn"><i class="ph-bold ph-arrow-right"></i></a>
        <h2 style="margin:0; font-size:1.6rem; color:#0f172a; font-weight:800;"><?= $isEdit ? 'تعديل إذن التسليم' : 'إصدار إذن تسليم بضاعة جديد' ?></h2>
    </div>
    ...
    <form action="<?= $actionUrl ?>" method="POST" id="deliveryForm">
        <div class="panel-card">
            <h3 style="margin: 0 0 20px 0; border-bottom: 1px solid #f1f5f9; padding-bottom: 10px; font-size:1.1rem; font-weight:800; color:#0f172a;"><i class="ph-duotone ph-truck" style="color:var(--c-amber);"></i> البيانات الأساسية للشحنة</h3>
            <div class="grid-2">
                <div>
                    <label class="input-label">العميل / الجهة المستلمة <span style="color:red">*</span></label>
                    <input type="text" name="customer_name" class="form-control" value="<?= $isEdit ? htmlspecialchars($note->customer_name) : '' ?>" placeholder="اسم الشركة أو العميل..." required>
                </div>
                <div>
                    <label class="input-label">المستودع المصدر <span style="color:red">*</span></label>
                    <select name="warehouse_id" class="form-control" required>
                        <option value="">-- اختر مستودع الصرف --</option>
                        <?php foreach($warehouses ?? [] as $w): ?>
                            <option value="<?= $w->id ?>" <?= ($isEdit && $note->warehouse_id == $w->id) ? 'selected' : '' ?>><?= htmlspecialchars($w->name_ar) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            ...
            <div class="grid-3" style="margin-top: 24px;">
                <div>
                    <label class="input-label">رقم إذن التسليم</label>
                    <input type="text" name="delivery_number" class="form-control" style="font-family:monospace; font-weight:bold; color:var(--c-amber-dark);" value="<?= $isEdit ? htmlspecialchars($note->delivery_number) : '' ?>" readonly placeholder="تلقائي">
                </div>
                <div>
                    <label class="input-label">تاريخ التسليم <span style="color:red">*</span></label>
                    <input type="date" name="delivery_date" class="form-control" value="<?= $isEdit ? htmlspecialchars($note->delivery_date) : date('Y-m-d') ?>" required>
                </div>
                <div>
                    <label class="input-label">حالة الإذن</label>
                    <select name="status" class="form-control" style="font-weight: 800;">
                        <?php $st = $isEdit ? $note->status : 'draft'; ?>
                        <option value="draft" <?= $st=='draft' ? 'selected' : '' ?>>مسودة (تجهيز)</option>
                        <option value="delivered" <?= $st=='delivered' ? 'selected' : '' ?>>تم التسليم للعميل (نهائي)</option>
                    </select>
                </div>
            </div>
            
            <div style="margin-top: 24px;">
                <label class="input-label">ملاحظات والتفاصيل</label>
                <input type="text" name="notes" class="form-control" value="<?= $isEdit ? htmlspecialchars($note->notes ?? '') : '' ?>" placeholder="ملاحظات الشحن، العنوان، السائق...">
            </div>
        </div>

        <div class="panel-card">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                <h3 style="margin:0; font-size:1.1rem; font-weight:800; color:#0f172a;"><i class="ph-duotone ph-list-checks" style="color:var(--c-amber);"></i> الاصناف المسلمة</h3>
            ...
            <table class="items-table" id="itemsTable">
                <thead>
                    <tr>
                        ...
                        <th style="width: 20%; text-align: center;">الكمية المسلمة <span style="color:red">*</span></th>
        ...
        <div class="sticky-footer">
            <a href="/ERP/inventory/delivery-notes" class="btn-cancel">إلغاء وتراجع</a>
            <button type="submit" class="btn-submit"><i class="ph-bold ph-floppy-disk"></i> <?= $isEdit ? 'تحديث البيانات' : 'حفظ وإصدار الإذن' ?></button>
        </div>
    </form>
</div>
