...
$deliveryNumber = htmlspecialchars($note->delivery_number ?? '---');
$deliveryDate   = htmlspecialchars($note->delivery_date ?? date('Y-m-d'));
$customerName   = htmlspecialchars($note->customer_name ?? '---');
$warehouseName  = htmlspecialchars($note->warehouse_name ?? '---');
$warehouseCode  = htmlspecialchars($note->warehouse_code ?? '---');
$notes          = htmlspecialchars($note->notes ?? '');
...
<div class="doc-wrapper" dir="<?= $dir ?>">
    <div class="top-bar">
        ...
        <button onclick="window.print()" class="btn-act"><i class="ph-bold ph-printer"></i> طباعة إذن التسليم</button>
    </div>
    ...
    <div class="print-canvas">
        <div class="c-header">
            <div>
                ...
                <p style="margin:4px 0 0 0; color:#64748b; font-weight:700;">إذن تسليم بضاعة (Delivery Note)</p>
            </div>
            <div style="text-align: <?= $isAr ? 'left' : 'right' ?>;">
                ...
                <div style="margin-top:6px; font-weight:800; color: #0f172a;">التاريخ: <?= $deliveryDate ?></div>
        ...
        <?php if(!empty($notes)): ?>
            <div style="margin-bottom: 24px; padding: 12px 16px; border-right: 4px solid var(--c-amber); background: #fef3c7; color: #b45309; font-weight: 700; border-radius: 6px;">
                <strong style="color: #92400e;">البيان / ملاحظات الشحن:</strong> <?= $notes ?>
            </div>
        <?php endif; ?>
        ...
        <table class="table-print">
            <thead>
                <tr>
                    ...
                    <th style="width: 14%; text-align: center;">الكمية المسلمة</th>
                </tr>
            </thead>
            <tbody>
                ...
                <?php endforeach; else: ?>
                    <tr>
                        <td colspan="5" style="text-align: center; padding: 20px; color: #94a3b8;">لا توجد أصناف مسجلة لهذا الإذن.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
            <tfoot>
